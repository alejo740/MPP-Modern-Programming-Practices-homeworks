package edu.mum.cs.cs401.labs.lab10.problem6;
...
import edu.mum.cs.cs401.labs.lab10.problem2.bugreporter.BugReportGenerator;
	...
	public void printListOfExpectedFemaleGuests(List<Guest> invited) {
		...
		try {
			String guests = invited
					.stream()
					.filter(g -> PredicateWithException.unchecked((Guest g2) -> checkLegal(g2)).test(g)
							&& g.getGender().equals(Gender.F) && g.isPlanningToCome())
					.map(g -> g.getName())
					.sorted()
					.collect(Collectors.joining(", "));
			System.out.println(guests);
		} catch (RuntimeException e) {
			Logger.getLogger(GuestListJava8.class.getName()).log(Level.INFO, e.getMessage());
		}
	}
